...
API_KEY = os.environ.get('BACK_UP_API_KEY')
...
class TradeDB(Base):
    ...
    kode_sumber = db.Column(mysql.CHAR(2),nullable=True) #2
...
class TradeModel(BaseModel):
    ...
    cmdCode :str #hscode
    qty:Optional[Decimal] = None #vol
    qtyUnitCode:Optional[Union[str,int]] = None #satuan --> ubah jadi kg/g
    primaryValue:Optional[Decimal] = None #tarif
    netWgt:Optional[Decimal] = None #nilai
    ...
    def sanitize_numbers(cls,v):
        if v == '' or v is None:
            return Decimal('0.0')
        return Decimal(str(f'{float(v):.2f}'))
    ...
    def _transform(self, raw_item:dict)->dict:
        try:
            ...
            return {
                ...
                'tarif' : 0.0,
                'nilai' : clean.primaryValue.quantize(Decimal('0.01'),rounding=ROUND_HALF_UP),
                ...
                'berat_bersih': clean.netWgt.quantize(Decimal('0.01'),rounding=ROUND_HALF_UP) if clean.netWgt != None else None,
                'pelabuhan': None,
                'hs_len':4
            }
...
def main():
    ...
    reporters_env_str =  os.getenv("REPORTER_CODE", "IDN")
    reporters_env = reporters_env_str.split(",")
    ...
    for reporter in reporters_env:
        print(f'\nMEMPRROSES NEGARA {reporter}')
        rCodeM49 = convert_country.to_m49(reporter)
        ...
        for curr_year in range(s_year,e_year+1):
            ...
            existing_hs = get_existing_code(session,reporter, year_str)
